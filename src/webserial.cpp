...
void myAppHandler(String* words, int totalWords) {
  ...
  if (command == "st") {
    if (totalWords > 1) {
      if (words[1] == "auto") { seatalk->sendCommand(auto_but); log::toAll("ST: auto"); }
      else if (words[1] == "standby") { seatalk->sendCommand(standby_but); log::toAll("ST: standby"); }
      else if (words[1] == "+1") { seatalk->sendCommand(plus_1); log::toAll("ST: +1"); }
      else if (words[1] == "-1") { seatalk->sendCommand(minus_1); log::toAll("ST: -1"); }
      else if (words[1] == "+10") { seatalk->sendCommand(plus_10); log::toAll("ST: +10"); }
      else if (words[1] == "-10") { seatalk->sendCommand(minus_10); log::toAll("ST: -10"); }
      else if (words[1] == "wind") { seatalk->sendCommand(wind_mode); log::toAll("ST: wind"); }
      else if (words[1] == "timer") { seatalk->sendCommand(start_timer); log::toAll("ST: timer"); }
      else log::toAll("st {auto|standby|+1|-1|+10|-10|wind|timer}");
    } else {
      log::toAll("st {auto|standby|+1|-1|+10|-10|wind|timer}");
    }
    return;
  }
  if (command == "wind") {
    extern void windClientStatus();
    windClientStatus();
    return;
  }
  if (command == "windtcp") {
...
static String commandList[] = {"restart", "hostname", "status", "wifi", "conslog", "littlefs", "toggle", "timer", "scan", ""};
